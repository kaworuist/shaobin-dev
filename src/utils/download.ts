...
export default {
	// 下载 Excel 方法
	excel(data: BlobPart, fileName: string) {
		this.download0(data, fileName, 'application/vnd.ms-excel');
	},

	// 下载 Word 方法
	word(data: BlobPart, fileName: string) {
		this.download0(data, fileName, 'application/msword');
	},

	// 下载 Zip 方法
	zip(data: BlobPart, fileName: string) {
		this.download0(data, fileName, 'application/zip');
	},

	// 下载 Html 方法
	html(data: BlobPart, fileName: string) {
		this.download0(data, fileName, 'text/html');
	},

	// 下载 Markdown 方法
	markdown(data: BlobPart, fileName: string) {
		this.download0(data, fileName, 'text/markdown');
	},

	download0(data: BlobPart, fileName: string, mineType: string) {
		// 创建 blob
